...
Parser::Parser() : token_index(0), line_count(1), error_count(0), in_function_scope(false) {}
...
bool Parser::isDataType(TokenType token) {
    return token == INTEGER || token == SINTEGER || token == CHARACTER ||
           token == STRING || token == FLOAT || token == SFLOAT || token == VOID ||
           token == STRUCT;
}

bool Parser::isStartOfStatement(TokenType type) {
    return type == IDENTIFIER || type == CONSTANT || type == STRING_LITERAL ||
           type == CHARACTER_LITERAL || (type == BRACE && (current_token.text == "(" || current_token.text == "{")) ||
...
void Parser::throwError(std::ofstream& out) {
    error_count++;
    std::cout << "Line : " << current_token.line << " Not Matched Error: Unexpected token '" << current_token.text << "'" << std::endl;
    out << "Line : " << current_token.line << " Not Matched Error: Unexpected token '" << current_token.text << "'" << std::endl;
...
void Parser::parseDeclarations(std::ofstream& out) {
    while (isDataType(current_token.type) || current_token.type == INCLUSION ||
           current_token.type == COMMENT_START || current_token.type == SINGLE_LINE_COMMENT_START) {
        if (current_token.type == INCLUSION) {
            parseIncludeCommand(out);
        } else if (current_token.type == COMMENT_START || current_token.type == SINGLE_LINE_COMMENT_START) {
            parseComment(out);
        } else {
    ...
    if (isDataType(current_token.type)) {
        ...
        if (current_token.type == IDENTIFIER) {
            ...
            if (current_token.type == BRACE && current_token.text == "(") {
                ...
                std::cout << "Line : " << start_line << " Matched Rule used: fun-declaration" << std::endl;
                out << "Line : " << start_line << " Matched Rule used: fun-declaration" << std::endl;
            } else if (current_token.type == BRACE && current_token.text == "{") {
                parseStructDec(out);
                std::cout << "Line : " << start_line << " Matched Rule used: struct-declaration" << std::endl;
                out << "Line : " << start_line << " Matched Rule used: struct-declaration" << std::endl;
            } else {
                parseVarDec(out);
                std::cout << "Line : " << start_line << " Matched Rule used: var-declaration" << std::endl;
                out << "Line : " << start_line << " Matched Rule used: var-declaration" << std::endl;
            }
        } else {
            throwError(out);
        }
...
void Parser::parseVarDec(std::ofstream& out) {
    if (current_token.type == IDENTIFIER) {
        if (!std::isalpha(current_token.text[0]) && current_token.text[0] != '_') {
            std::cout << "Line : " << current_token.line << " Not Matched Error: Invalid identifier \"" << current_token.text << "\"" << std::endl;
            out << "Line : " << current_token.line << " Not Matched Error: Invalid identifier \"" << current_token.text << "\"" << std::endl;
            error_count++;
            nextToken();
        } else {
            nextToken();
        ...
        if (current_token.type == ASSIGNMENT_OP) {
            if (!in_function_scope) {
                std::cout << "Line : " << current_token.line << " Not Matched Error: Variable initialization only allowed inside function" << std::endl;
                out << "Line : " << current_token.line << " Not Matched Error: Variable initialization only allowed inside function" << std::endl;
                error_count++;
                nextToken();
                parseExpression(out);
            } else {
...
void Parser::parseStatement(std::ofstream& out) {
    ...
    switch (current_token.type) {
        ...
        case CHARACTER_LITERAL:
            ...
            std::cout << "Line : " << start_line << " Matched Rule used: expression-stmt" << std::endl;
            out << "Line : " << start_line << " Matched Rule used: expression-stmt" << std::endl;
            break;
        case BRACE:
            if (current_token.text == "(") {
                ...
                parseCompoundStmt(out);
                std::cout << "Line : " << start_line << " Matched Rule used: compound-stmt" << std::endl;
                out << "Line : " << start_line << " Matched Rule used: compound-stmt" << std::endl;
            } else {
                throwError(out);
            }
            break;
        case CONDITION:
            ...
            std::cout << "Line : " << start_line << " Matched Rule used: selection-stmt" << std::endl;
            out << "Line : " << start_line << " Matched Rule used: selection-stmt" << std::endl;
            break;
        case LOOP:
            parseIterationStmt(out);
            std::cout << "Line : " << start_line << " Matched Rule used: iteration-stmt" << std::endl;
            out << "Line : " << start_line << " Matched Rule used: iteration-stmt" << std::endl;
            break;
        case RETURN:
        case BREAK:
            parseJumpStmt(out);
            std::cout << "Line : " << start_line << " Matched Rule used: jump-stmt" << std::endl;
            out << "Line : " << start_line << " Matched Rule used: jump-stmt" << std::endl;
            break;
        default:
            throwError(out);
    }
...
void Parser::parseUnsignedNum(std::ofstream& out) {
    parseValue(out);
}
...
void Parser::parseComment(std::ofstream& out) {
    ...
    if (current_token.type == COMMENT_START) {
        ...
        if (current_token.type == COMMENT_END || current_token.type == INVALID_COMMENT) {
            nextToken();
        } else {
            throwError(out);
        }
    ...
    std::cout << "Line : " << start_line << " Matched Rule used: Comment" << std::endl;
...
void Parser::parseIncludeCommand(std::ofstream& out) {
    ...
    if (current_token.type == INCLUSION) {
        nextToken();
        if (current_token.type == STRING_LITERAL || current_token.type == INVALID_INCLUSION) {
            parseFName(out);
            if (current_token.type == SEMICOLON) {
                nextToken();
                std::cout << "Line : " << start_line << " Matched Rule used: include_command" << std::endl;
                out << "Line : " << start_line << " Matched Rule used: include_command" << std::endl;
            } else {
                throwError(out);
            }
        } else {
